...
export function JoinTypePopover({ currentType, onSelect, onClose, style }: JoinTypePopoverProps) {
  ...
  return (
    <div
      ref={popoverRef}
      style={style}
      className="absolute z-20 w-[192px] rounded-md border border-border-default bg-bg-overlay p-1.5 shadow-lg"
    >
        ...
        {JOIN_TYPE_OPTIONS.map((joinType) => {
          ...
          return (
            <button
              ...
              className={`flex flex-col items-center rounded-md border px-1 py-1 text-[9px] font-mono ${active ? 'border-border-focus bg-accent-dim' : 'border-border-subtle bg-bg-elevated hover:bg-bg-hover'} ${colorClass}`}
            >
